...
impl<const N: usize, const S: usize> FastMem<N, S> {
    ...
    // - OOM results in None
    #[inline(always)]
    fn alloc<T>(&'static self) -> Option<(&mut T, usize)> {
        ...
        if new_start > self.end.get() {
            ...
            let r: &mut T = unsafe { transmute::<_, &mut T>(new_start) };
    ...
    pub fn try_new<T>(&'static self, t: T) -> Option<Box<T>> {
        ...
        let node: &Node<T> = match stack.pop() {
            ...
            None => {
                // new allocation
                let (data, index): (&mut T, _) = self.alloc()?;
        ...
        let data_ptr: &mut T = node.as_mut_ref();
        *data_ptr = t;
